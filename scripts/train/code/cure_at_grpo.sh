...
python3 -m pettingllms.trainer.train --config-path ../config/code --config-name code_L1_lora \
    $model_0_resource \
    +roll_mode=tree\
    +if_save=True\
    base_models.policy_0.path="/home/lah003/models/Qwen2.5-7B-Instruct"\
    training.experiment_name=code_8B_qwen2.5-7B-Instruct\
    ...
    env.dataset=code_contests_cure\
    env.benchmark=livecodebench_cure\