...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"autoeval_mix_${DESIGN_SAMPLE_NUM}d_${EXECUTE_SAMPLE_NUM}e_4kdesign_only"}
...
python -m pettingllms.trainer.train --config-path ../config/autoevol --config-name math_L1_prompt \
    ...
    training.executor_lr=1e-9\
    training.lr_alternate_steps=300\
    env.name=mixed_env\
