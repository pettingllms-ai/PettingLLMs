...
# Hard-pin FLASH_ATTN backend.
# Reason: FLASHINFER backend triggers vLLM JIT compile via ninja+nvcc at
# unpredictable points during training. The ray worker's PATH points
# nvcc at /opt/conda/envs/sandbox-runtime/.../triton/.../bin/nvcc which
# does not exist in the worker namespace -> ninja FAILED [code=127] ->
# EngineDeadError -> training crashes (observed at step 80 in
# 0416_mix_try_run.log). FLASH_ATTN ships precompiled kernels, no JIT.
# Trade-off: ~10-20% slower attention vs FLASHINFER, but never crashes.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"autoeval_mix_4d_4e_mix_question_grouping_altlr_zerolr_30steps_firstexe"}
