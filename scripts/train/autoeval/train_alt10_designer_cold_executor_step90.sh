...
COLD_START_PATH=${COLD_START_PATH:-"Mercury7353/masrl_0228_mix_coldstart"}
APPS_RATIO=${APPS_RATIO:-0.7}
DESIGNER_LR=${DESIGNER_LR:-5e-6}
EXECUTOR_LR=${EXECUTOR_LR:-5e-6}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"autoeval_iterated_2policy_alt10_designer_cold_executor_step90"}
CKPT_DIR=${CKPT_DIR:-"/mnt/afs/video_ckpt/agent/zhangyaolun/masrl"}

# Executor resumes from this old run's step 90 (which contains the trained
# executor weights — designer's were overwritten by the pre-fix save bug).
EXECUTOR_RESUME_PATH=${EXECUTOR_RESUME_PATH:-"/mnt/afs/video_ckpt/agent/zhangyaolun/masrl/autoeval_iterated_2policy_30step_firstexe/global_step_90"}

# IBR phase logic at step 90: phase = (90 // 10) % 2 = 1.
# With first_phase_trains=executor: phase 0 trains executor, phase 1 trains designer.
# So at step 90 we will train DESIGNER first (let it catch up to the trained executor).
...
echo "Phase alternate: every $PHASE_ALTERNATE_STEPS steps, first_phase=$FIRST_PHASE_TRAINS"

exec "$PY" -u -m pettingllms.trainer.train --config-path ../config/autoevol --config-name math_L1_iterated_br \
    $model_0_resource \
    $model_1_resource \
    base_models.policy_0.path="$COLD_START_PATH"\
    base_models.policy_1.path="$COLD_START_PATH"\
    lora_rank=0\
    lora_alpha=16\
    ...
    +$model_0_config_path.checkpoint_dir=$CKPT_DIR\
    $model_1_config_path.trainer.resume_mode=resume_path\
    $model_1_config_path.trainer.resume_from_path="$EXECUTOR_RESUME_PATH"\
    ...
    +$model_1_config_path.checkpoint_dir=$CKPT_DIR


# What this script does:
#   - Designer (model_0): initialized from cold-start ($COLD_START_PATH).
#     resume_mode=disable, so the trainer never tries to load any ckpt and
#     always starts from base_models.policy_0.path.
#   - Executor (model_1): resumes from the previous broken run's step_90
#     ($EXECUTOR_RESUME_PATH). Old ckpt has executor weights at that path
#     (under .../global_step_90/actor) — _load_checkpoint will fall back
#     from per-policy subdir to the legacy flat path automatically.
#   - global_steps starts at 90 (set by executor's resume).
#   - phase_alternate_steps=10, first_phase_trains=executor:
#       step 90-99   phase 1 → train DESIGNER (executor frozen)
#       step 100-109 phase 0 → train EXECUTOR (designer frozen)
#       step 110-119 phase 1 → train DESIGNER
#       ... alternates every 10 steps
#   - Save (with the per-policy subdir fix in ray_trainer.py):
#       step_100/designer_policy/actor   ← preserved
#       step_100/executor_policy/actor   ← preserved
#     No more overwriting between the two policies.
