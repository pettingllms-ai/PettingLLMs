...
class SampleReasoningAgent(Agent):
    """
    Agent specialized for solving mathematical problems with multiple sampling support.
    Can generate sample_num different solutions per step.
    """

    def __init__(self, rollout_idx: int | None = None, **kwargs):
        """
        Initialize the Sampling Reasoning Agent's data.
        """
        super().__init__()
    ...
    def update_from_env(self, turn_idx: int, env_data: Env):
        ...
        reasoning_generated_solution_history = getattr(state, "reasoning_generated_solution_list", None)
        reasoning_extracted_answer_history = getattr(state, "reasoning_extracted_answer_list", None)
        code_generated_solution_history = getattr(state, "code_generated_solution_list", None)
        code_extracted_answer_history = getattr(state, "code_extracted_answer_list", None)
        ...
        if turn_idx == 0:
            ...
            formatted_prompt = (
                f"You are a helpful assistant that selects or refines the best mathematical solutions through reasoning.\n\n"
                f"Problem:\n{problem}\n\n"
            )
            ...
            formatted_prompt += (
                ...
                f"Then solve the problem again. Over all, if the computation is too complex, the reasoning might be more easier to make mistake.\n"
            )
            formatted_prompt += (
    ...
    def update_from_model(self, response: str):
        """
        Store the model's response. Can be a single response or multiple responses.
        """
    ...
    async def step(self, env_data: Env, env_worker: Any = None):
        ...
        env_data.state.reasoning_generated_solution_list.append(env_data.state.reasoning_generated_solution)
        env_data.state.reasoning_extracted_answer_list.append(env_data.state.reasoning_extracted_answer)
