...
def prompt_code_sokoban(turn_idx: int, state: Any) -> str:
    ...
    return (
        ...
        "Algorithm hints (optional):\n"
        "- Model state as (player_pos, boxes) and treat pushes as macro-actions; search push sequences with BFS/A*.\n"
        "- Before each push, plan a path for the player to the push position with BFS (treat boxes as obstacles).\n"
        "- Heuristic: sum of Manhattan distances from boxes to nearest goals; prefer moves that increase boxes-on-goals.\n"
        ...
        "    # TODO: implement BFS/A* over pushes; here we return an empty plan as a placeholder\n"
        "    return []\n\n"
        "if __name__ == \"__main__\":\n"
        "    obs = \"\"\"\n"
        f"{observation}\n"
        "\"\"\"\n"
        "    actions = solve_sokoban(obs)\n"
