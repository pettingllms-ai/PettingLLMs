...
class MathEnvBatch:
    def __init__(self, env_idx_list: List[int],env_indices: List[int], rollout_idx_list: List[int], samples: int, max_turns: int, config: dict, mode="train", *, env_workers: List = None):
        ...
        dataset_name=getattr(config.env,"dataset")
        self.problem_list = load_math_problem_batch(env_indices, mode=mode,dataset_name=dataset_name, config=config,benchmark_name=benchmark_name)
        self.env_list = []
        if mode == "validate":
