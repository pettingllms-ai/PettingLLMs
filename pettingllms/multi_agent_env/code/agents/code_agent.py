...
class CodeGenerationAgent(Agent):
    ...
    def update_from_env(self, env_data: Env):
        ...
        if match_cases is not None:
            formatted_prompt_for_match_cases += "The following test cases successfully align with the code implementation generated by the other language model:\n"
        ...
        if need_generate:
            ...
            formatted_prompt = (
                ...
                f"But the execution result of the current code is not aligned with the given test case outputs generated by another LLM. To be noted, the test case outputs are generated by another LLM, so they may not be correct.\n")
            formatted_prompt += formatted_prompt_for_mismatch_cases + (
                f"Please first judge the mismatch between the current generated test cases and the current code execution result, if the mismatch is caused by the current code, please refine the code to pass all tests. If the mismatch is not caused by the current code, please answer the original code.\n"
