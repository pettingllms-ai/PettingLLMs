...
class UnitTestGenerationAgent(Agent):
    ...
    def update_from_env(self, env_data: Env):
        ...
        if need_generate:
            ...
            formatted_prompt += (
                ...
                f"but the execution result is not aligned with the code execution outputs where the code are generated by another large language model.\n")
                
            formatted_prompt += formatted_prompt_for_match_cases + formatted_prompt_for_mismatch_cases + (
        ...
        if gen_inputs and gen_outputs and getattr(env_data.state, "generated_code", None):
            try:
                env_passed_ratio, env_passed_cases, env_failed_cases = await evaluate_code_against_tests(
                    env_data.state.generated_code, gen_inputs, gen_outputs, timeout=60.0,ray_actor=env_worker,rollout_idx=self.rollout_idx
                )
    ...
    def calculate_reward(self, env_data: List[Env]) -> float:
        ...
        if len(self.reward_history)>0:
            self.agent_reward = pass_ratio-self.reward_history[-1]
        else:
